...
pub fn handler<'info>(ctx :Context<'info,TransferWithHook<'info>>, amount:u64,decimals:u8) ->Result<()>{
    
        //accounts from ctx.account
    ...
    // Token-2022 transfer_checked instruction
    ...
    // List the AccountInfos required by the instruction
    let mut infos = vec![
        source.clone(),
        mint.clone(),
        destination.clone(),
