...
pub fn setup() -> (LiteSVM, Keypair, Address) {
    ...
    let bytes = include_bytes!("../../../../target/deploy/solana_fall_transfer_hook.so");  //added
    svm.add_program(program_id, bytes).unwrap();   //added

    let token_mover_bytes = include_bytes!("../../../../target/deploy/token_mover.so");   //added
    svm.add_program(token_mover::id(), token_mover_bytes).unwrap();      //added
...
pub fn initialize_rate_limit(svm: &mut LiteSVM, payer: &Keypair, mint: &Keypair, program_id: &Address) {
    let rate_limit = Pubkey::find_program_address(
        &[b"rate_limit", mint.pubkey().as_ref(), payer.pubkey().as_ref()],   //added
        program_id,
    ).0;
...
pub fn build_transfer_with_hook_ix(
    ...
    let rate_limit = Pubkey::find_program_address(
        &[b"rate_limit", mint.as_ref(), owner.as_ref()],     //added
        program_id,
    ).0;
...
pub fn build_transfer_with_token_mover_ix(
    source_ata: &Pubkey,
    dest_ata: &Pubkey,
    mint: &Pubkey,
    owner: &Pubkey,
    ...
    ix.accounts
        .push(AccountMeta::new(rate_limit, false));
